...
- Location: DataAccessLayers\MSSqlServer\Templates\Repository_MigrationRun_Update.sql

Function:
- Updates the status/result of a migration-run in the RayMigrator repository.
...
BEGIN TRY
	...
	SET
		[{CFG:TableBaseName}MigrationRunResultId] = @MigrationRunResult,
	...
	SELECT '0,MigrationRun with Id [' + ISNULL(CAST(@MigrationRunId as varchar(10)), 'NULL') + N'] successfully updated with MigrationRunResultId [' + ISNULL(CAST(@MigrationRunResult as varchar(10)), 'NULL') + N']';
...
BEGIN CATCH
	...
	SELECT '-99,Error executing MSSqlServer\Templates\Repository_MigrationRun_Update.sql: Could NOT update existing MigrationRun-entry in RayMigrator repository. ErrorInfo: ' + @ErrorInfo;
